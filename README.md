...
| 檔 | kind | 來源 | 怎麼產 |
| --- | --- | --- | --- |
| `rules.json` | `rules` | App 內建的營養素（含國健署 DRIs 第八版分族群參考值）、化學形式、藥物類別、搭配規則、健康狀況 | App repo `tools/export-rules.sh`（Swift 是唯一原始來源，**不要手改**；改資料回 App 改 Swift、`Rules.builtinVersion` +1） |
